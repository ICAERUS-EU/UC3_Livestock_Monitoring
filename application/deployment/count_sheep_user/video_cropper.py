...
class VideoCropper(QObject):
    ...
    def crop_and_export_video(self):
        ...
        ret, frame = cap.read()
        if not ret:
            QMessageBox.critical(self, "Erreur", "Impossible de lire la première frame.")
            cap.release()
            return
        ...
        if crop_x + crop_w > video_width or crop_y + crop_h > video_height:
            QMessageBox.critical(self, "Erreur", "La zone de crop dépasse les dimensions de la vidéo.")
            cap.release()
            return
        ...
        QMessageBox.information(self.parent, "Succès", f"La vidéo a bien été exportée !\nEmplacement :\n{crop_video_path}")

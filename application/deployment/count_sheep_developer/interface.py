...
class InterfaceApp(QWidget):
    def __init__(self):
        ...
        # FPS selection
        self.fps_label = QLabel("Choisissez le FPS de sortie :")
        self.fps_label.setStyleSheet("font-size: 16px;")
        self.fps_label.setAlignment(Qt.AlignCenter)

        self.radio_5fps = QRadioButton("5 FPS")
        self.radio_15fps = QRadioButton("15 FPS")
        self.radio_30fps = QRadioButton("30 FPS")
        self.radio_15fps.setChecked(True)  # Default value

        self.fps_group = QButtonGroup(self)
        self.fps_group.addButton(self.radio_5fps, 5)
        self.fps_group.addButton(self.radio_15fps, 15)
        self.fps_group.addButton(self.radio_30fps, 30)

        self.fps_group.buttonClicked.connect(self.update_fps_selection)
        ...
        fps_layout.addWidget(self.radio_5fps)
        fps_layout.addWidget(self.radio_15fps)
        fps_layout.addWidget(self.radio_30fps)
        fps_layout.addStretch()
    ...
    def update_model_selection(self, id):
        # Update the selected model path when a radio button is clicked
        self.model_path, self.model_suffix = self.model_paths[id]


    def update_fps_selection(self, button):
        # Update the selected FPS value when a radio button is clicked
        self.selected_fps = self.fps_group.id(button)
    ...
    def start_loading(self):
        self.progress_dialog = QProgressDialog("Traitement de la vidéo...", None, 0, 0) # Create progress dialog
